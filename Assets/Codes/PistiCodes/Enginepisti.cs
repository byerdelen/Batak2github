...
public class Enginepisti : MonoBehaviour
{
    ...
    public void Start()
    {
        string[] tempdata = PlayerPrefs.GetString("eslipistidata").Split(","[0]);
    ...
    {
        PlayerPrefs.SetString("eslipistidata", "0,0,0,0,0," + val);
        totalhands = val;
        hand = 1;
        StartCoroutine(starting());
    }
    ...
    public void gameended()
    {
        ...
        scoresnow[0].text = "Sonuç : " + (points[0] + points[2]);
        scoresnow[1].text = "Sonuç : " + (points[1] + points[3]);
        scores[0].text = "Toplam Puan : " + (prevpoints[0] + points[0] + points[2]);
        scores[1].text = "Toplam Puan : " + (prevpoints[1] + points[1] + points[3]);
        ...
        if (hand != totalhands)
        {
            ...
            if (tempdata.Length > 1)
            {
                PlayerPrefs.SetString("eslipistidata", "" + hand + "," + (prevpoints[0] + points[0] + points[2]) + "," + (prevpoints[1] + points[1] + points[3]) + "," + totalhands);
                handstatewrite();
            }
        }
        else
        {
            int temprank = ((prevpoints[0] + points[0] + points[2]) > (prevpoints[1] + points[1] + points[3])) ? 1 : 2;
            PlayerPrefs.DeleteKey("eslipistidata");
