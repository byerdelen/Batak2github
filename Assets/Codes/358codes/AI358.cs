...
public class AI358 : MonoBehaviour
{
    ...
    public IEnumerator addinfo(Card tempcard)
    {
        cards.Add(tempcard);
    ...
    IEnumerator play()
    {
        ...
        if (engine.middle.cardcount() > 0)
        {
            ...
            if (Cardstatic.checkifihavethecardtype(engine.middle.startcard.type, cards))
            {
                ...
                else if (engine.middle.cardcount() == 2 && Cardstatic.findbiggestofthetype(engine.middle.startcard.type, false, cards) > Cardstatic.findbiggestfromtypetonumber(engine.middle.startcard.type, engine.middle.cards))
                {
